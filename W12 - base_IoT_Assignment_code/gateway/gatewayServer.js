...
function handle_incoming_request (req, res) {
	...
	// Need to adjust xml text string readings using an array
	...
	if(req.method == 'GET')
	{
		...
		if (err) { 
			...
			res.writeHead(200, { "Content-Type" : "text" });	// Text or json?? res.writeHead(200, { "Content-Type" : "application/text" });
			res.end(readings); 
		});
	}
}
...
function write_sensor_data(readingsToWrite) {
   ...
   fs.appendFile(
	...
	"sensorlog.txt",readingsToWrite + '\n', function(err) {		// 	"sensorlog.txt",readingsToWrite + '\n', function(err) {
		if(err) {
